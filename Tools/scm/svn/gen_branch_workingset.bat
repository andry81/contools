...
if %SVN_BRANCH_PATH_IS_WC_URL% NEQ 0 (
  call "%%?~dp0%%extract_info_param.bat" "%%BRANCH_ROOT_INFO_FILE_TMP%%" "Working Copy Root Path" || (
...
if %FLAG_SVN_DIFF_RA% NEQ 0 (
  call "%%?~dp0%%extract_info_param.bat" "%%BRANCH_ROOT_INFO_FILE%%" "Last Changed Rev" || (
...
call "%%?~dp0%%extract_info_param.bat" "%%BRANCH_ROOT_INFO_FILE%%" "Revision"
...
if exist "%BRANCH_ROOT_CHANGESET_FILE_DIR%" (
  ...
  if %SVN_BRANCH_PATH_IS_WC_URL% NEQ 0 (
    ...
    pushd "%SVN_BRANCH_PATH%" && (
      call "%%TOOLS_PATH%%/scm/svn/svn_changeset.bat" -r "!%%BRANCH_ROOT_CURRENT_REV%%" -wcroot "%%SVN_BRANCH_WCROOT_PATH%%" > "%BRANCH_ROOT_CHANGESET_FILE%" || ( popd & exit /b 24 )
      popd
    )
  )
)
...
if %FLAG_SVN_LIST_FILES% NEQ 0 (
  ...
  if not "%BRANCH_EXTERNAL_BASE_PATH%" == "" (
    type "%BRANCH_ROOT_FILES_FILE%" | "%TOOLS_PATH%/gnuwin32/bin/sed.exe" -e "s|^|%BRANCH_EXTERNAL_BASE_PATH%/|" >> "%BRANCH_ROOT_ALLFILES_FILE%"
  ) else (
...
set /P ICONV_DEFAULT_ANSI_LOCALE=< "%TOOLS_PATH%/vars/iconv.default_ansi_locale.var"

call "%%TOOLS_PATH%%/encoding/ansi2any.bat" "%%ICONV_DEFAULT_ANSI_LOCALE%%" UTF-8 "%%BRANCH_ROOT_ALLFILES_FILE%%" > "%BRANCH_ROOT_ALL_FILES_LIST_FILE_TMP%"
...
call "%%TOOLS_PATH%%/gen_dir_hash_lst.bat" -j 0 -c "md5,sha256" -l -f "%%BRANCH_ROOT_ALLFILES_FILE%%" 2>nul > "%BRANCH_ROOT_ALLFILES_DIR%%BRANCH_ROOT_ALLFILES_NAME%_hash.lst"
echo.
...
call "%%?~dp0%%extract_info_param.bat" "%%BRANCH_INFO_FILE%%" "Path"
...
type "%~dpf1" | "%TOOLS_PATH%/gnuwin32/bin/sed.exe" -b -e "s|%SED_SEARCH_STR%|%SED_REPLACE_STR%|img" > "%BRANCH_EXTERNALS_INFO_FILE_TMP%"
...
call "%%?~dp0%%extract_info_param.bat" "%%BRANCH_WORKINGSET_CATALOG_PATH%%/$info.txt" "Revision"
set "BRANCH_EXTERNAL_CURRENT_REV=%RETURN_VALUE%"
if "%BRANCH_EXTERNAL_CURRENT_REV%" == "" (
...
call "%%?~dp0%%extract_info_param.bat" "%%BRANCH_WORKINGSET_CATALOG_PATH%%/$info.txt" "Last Changed Rev"
set "BRANCH_EXTERNAL_LAST_REV=%RETURN_VALUE%"
if "%BRANCH_EXTERNAL_LAST_REV%" == "" (
...
if %SVN_BRANCH_PATH_IS_WC_URL% NEQ 0 (
  ...
  pushd "%BRANCH_EXTERNAL_WORKING_PATH%" && (
    call "%%TOOLS_PATH%%/scm/svn/svn_changeset.bat" -r "!%%BRANCH_EXTERNAL_CURRENT_REV%%" > "%BRANCH_WORKINGSET_CATALOG_PATH%/$changeset.lst" || ( popd & exit /b 50 )
    popd
  )
)
...
if %FLAG_SVN_LIST_FILES% NEQ 0 (
  ...
  if %SVN_BRANCH_PATH_IS_WC_URL% NEQ 0 (
    rem set a current directory for "svn ls" command to reduce path lengths in output
    pushd "%BRANCH_EXTERNAL_WORKING_PATH%" && (
      call "%%TOOLS_PATH%%/scm/svn/svn_list.bat" %%FLAG_TEXT_SVN_OFFLINE%% . --depth infinity --non-interactive > "%BRANCH_WORKINGSET_CATALOG_PATH%/$files.lst" 2>nul
      popd
    )
  ) else (
...
if %FLAG_SVN_LIST_FILES% NEQ 0 (
  type "%BRANCH_WORKINGSET_CATALOG_PATH:/=\%\$files.lst" | "%TOOLS_PATH%/gnuwin32/bin/sed.exe" -e "s|^|%BRANCH_EXTERNAL_DIR_PATH_PREFIX2%%BRANCH_EXTERNAL_DIR_PATH%/|" >> "%BRANCH_ROOT_ALLFILES_FILE%"
)
...
call "%%?~dp0%%gen_externals_list.bat" "%%BRANCH_EXTERNALS_FILE%%" "%%BRANCH_REPO_ROOT%%" "%%BRANCH_DIR_URL%%" > "%BRANCH_EXTERNALS_LIST_FILE_TMP%"
if %ERRORLEVEL% NEQ 0 (
...
call "%%TOOLS_PATH%%/scm/index_patch_file.bat" BRANCH_DIFF_FILE_INDEX "%%BRANCH_DIFF_FILE%%"
...
call "%%TOOLS_PATH%%/xcopy_file.bat" "%%BRANCH_BINARY_FILE_DIR%%" "%%BRANCH_BINARY_FILE_NAME%%" "%%BRANCH_BINARY_DIFF_DIR_PATH%%" /NJS || goto :EOF
