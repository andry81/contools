...
:TEST
set "STRING_OFFSET=%~1"
call :CMD "%%TOOLS_PATH%%\strchr.bat" /v "" "" %%2
...
if %STRING_OFFSET% EQU %LASTERRORLEVEL% (
  ...
  "%TOOLS_PATH%\printf.exe" "PASSED: %__COUNTER1%.%__COUNTER2%: (%STRING_OFFSET% == %LASTERRORLEVEL%) FOUND_CHAR=`${FOUND_CHAR}` STRING=`${__STRING__}` CHARS=`${__CHARS__}`"
) else (
  rem print string containing __STRING__ and __CHARS__ environment variable value which may hold batch control characters
  "%TOOLS_PATH%\printf.exe" "FAILED: %__COUNTER1%.%__COUNTER2%: (%STRING_OFFSET% == %LASTERRORLEVEL%) FOUND_CHAR=`${FOUND_CHAR}` STRING=`${__STRING__}` CHARS=`${__CHARS__}`"
)
