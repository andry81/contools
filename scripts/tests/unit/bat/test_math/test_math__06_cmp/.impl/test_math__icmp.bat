...
rem common implementation for `icmp.bat` and `icmp_nvar.bat`
...
if "%TEST_IMPL_SCRIPT_NAME%" == "icmp_nvar.bat" (
  setlocal
  set RETREF=-1
  set "L="
  call :TEST L
...
rem      call icmp.bat 10 GTR 9 && echo TRUE || echo FALSE
rem      rem TRUE
setlocal
set RETREF=0
...
for /L %%i in (1,1,20) do (
  setlocal
  set RETREF=0
  call :TEST_SELECT %%~a%%ZEROS:~0,%%i%% EQU %%~a%%ZEROS:~0,%%i%%
  endlocal

  setlocal
  set RETREF=1
  call :TEST_SELECT %%~a%%ZEROS:~0,%%i%% NEQ %%~a%%ZEROS:~0,%%i%%
  endlocal

  setlocal
  set RETREF=1
  call :TEST_SELECT %%~a%%ZEROS:~0,%%i%% GTR %%~a%%ZEROS:~0,%%i%%
  endlocal

  setlocal
  set RETREF=0
  call :TEST_SELECT %%~a%%ZEROS:~0,%%i%% GEQ %%~a%%ZEROS:~0,%%i%%
  endlocal

  setlocal
  set RETREF=1
  call :TEST_SELECT %%~a%%ZEROS:~0,%%i%% LSS %%~a%%ZEROS:~0,%%i%%
  endlocal

  setlocal
  set RETREF=0
  call :TEST_SELECT %%~a%%ZEROS:~0,%%i%% LEQ %%~a%%ZEROS:~0,%%i%%
  endlocal
)
...
:TEST_SELECT
if "%TEST_IMPL_SCRIPT_NAME%" == "icmp.bat" goto TEST
