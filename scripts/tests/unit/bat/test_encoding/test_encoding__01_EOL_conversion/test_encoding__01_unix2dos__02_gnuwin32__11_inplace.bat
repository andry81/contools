...
call "%%CONTOOLS_ROOT%%/std/canonical_path.bat" TESTS_LIST_FILE     "%%TESTS_PROJECT_ROOT%%/test_encoding/.tests/01_mixed/01_unix2dos.lst"
...
for /f "usebackq eol=# tokens=* delims=" %%i in ("%TESTS_LIST_FILE%") do call :TEST "01_mixed\%%i" -i -fix-tail-lr

goto END

:SKIP_GNUWIN32
echo;SKIPPED: CONTOOLS_GNUWIN32_ROOT is not defined or not exist
echo;
