...
call "%%CONTOOLS_ROOT%%/std/setshift.bat" 1 TEST_DATA_CONV_SCRIPT_CMD_LINE %%*

call "%%CONTOOLS_ROOT%%/std/allocate_temp_dir.bat" . "%%TEST_SCRIPT_FILE_NAME%%" "" "%%TEST_DATA_TEMP_ROOT%%" || exit /b

set "TEST_TEMP_DIR_NAME=%SCRIPT_TEMP_ROOT_DATE%.%SCRIPT_TEMP_ROOT_TIME%.%TESTLIB__TEST_ORDER_NUMBER%.%TEST_SCRIPT_FILE_NAME%"
set "TEST_TEMP_DIR_PATH=%SCRIPT_TEMP_CURRENT_DIR%"

rem initialize setup parameters
call "%%CONTOOLS_ROOT%%/std/canonical_path.bat" TEST_TEMP_DATA_INOUT_FILE "%%TEST_TEMP_DIR_PATH%%\inout.txt"
