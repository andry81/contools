...
call "%%CONTOOLS_ROOT%%/std/cmp_var.bat" "%%TEST_VAR_NAME%%" EQU "%%REFERENCE_VAR_NAME%%" && (
  echo;!TESTLIB__TEST_STATUS_MSG_PASSED!: !TESTLIB__TEST_ORDER_NUMBER!''!TEST_VAR_CASE_INDEX!: VAR=`!TEST_VAR_NAME!` RESULT=`!%TEST_VAR_NAME%!`
  call;
) || (
  set /A "TEST_IMPL_ERROR|=0x80000000"
  echo;!TESTLIB__TEST_STATUS_MSG_FAILED!: !TESTLIB__TEST_ORDER_NUMBER!''!TEST_VAR_CASE_INDEX!: VAR=`!TEST_VAR_NAME!` RESULT=`!%TEST_VAR_NAME%!` REFERENCE=`!%REFERENCE_VAR_NAME%!`
  echo;
)
