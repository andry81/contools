...
set FLAG_BACKUP_DOWNLOADS=0
set FLAG_BACKUP_LOGS=0
set FLAG_BACKUP_PART_MET=0
set FLAG_BACKUP_SEARCHES=0
set FLAG_BACKUP_NETWORK_CACHE=0
set FLAG_BACKUP_NETWORK_FILTERS=0
set BACKUP=0
...
if defined FLAG (
  if "%FLAG%" == "-backup-all" (
    ...
    set BACKUP=1
  ) else if "%FLAG%" == "-backup-downloads" (
    set FLAG_BACKUP_DOWNLOADS=1
    set BACKUP=1
    ...
    set BACKUP=1
  ) else if "%FLAG%" == "-backup-searches" (
    set FLAG_BACKUP_SEARCHES=1
    set BACKUP=1
  ) else if "%FLAG%" == "-backup-network-cache" (
    set FLAG_BACKUP_NETWORK_CACHE=1
    set BACKUP=1
  ) else if "%FLAG%" == "-backup-network-filters" (
    set FLAG_BACKUP_NETWORK_FILTERS=1
    set BACKUP=1
  )
...
if %FLAG_BACKUP_DOWNLOADS% NEQ 0        call "%?~dp0%backup_downloads.bat" || exit /b
...
if %FLAG_BACKUP_SEARCHES% NEQ 0         call "%?~dp0%backup_searches.bat" || exit /b
if %FLAG_BACKUP_NETWORK_CACHE% NEQ 0    call "%?~dp0%backup_network_cache.bat" || exit /b
if %FLAG_BACKUP_NETWORK_FILTERS% NEQ 0  call "%?~dp0%backup_network_filters.bat" || exit /b
