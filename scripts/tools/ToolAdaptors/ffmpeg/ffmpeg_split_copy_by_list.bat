...
rem   -mode | -m
rem     Split mode:
rem       * by-file-size-limit-using-ss
...
rem         Faster than using `by-file-size-limit-using-ss` method because of a
rem         single call.
rem
rem         NOTE:
...
rem
rem     Does not imply `-force` flag, ignores an output chunk file write and
rem     issues a warning instead of an error.
rem
rem   -skip-current-on-ff-error
...
rem     file.
rem
rem   -output-rename-retry-count
rem     Controls the number of attempts to rename the output chunk file name.
rem     By default the number of attempts to rename is 3.
rem     Must be greater than 3.
rem
...
rem script flags
set "FLAG_MODE=by-file-size-limit"
...
if not defined FLAG_MODE set "FLAG_MODE=by-file-size-limit"

if not "%FLAG_MODE%" == "by-file-size-limit" (
...
call "%%CONTOOLS_ROOT%%/math/ucmp_fnvar.bat"  FLAG_FILE_OVERHEAD_KB_FN GEQ FLAG_FILE_SIZE_HALF_LIMIT_KB_FN && (
  ...
  echo;%?~%: info: File Size:      %FLAG_FILE_SIZE_LIMIT_MB% Mb.
  exit /b 255
) >&2
...
rem check `by-file-size-limit` mode environment
if not "%FLAG_MODE%" == "by-file-size-limit" goto SKIP_CHECK_MODE_BY_FILE_SIZE_LIMIT
