...
if %NO_DEFAULT_FLAGS% NEQ 0 (
  call "%%CONTOOLS_BUILD_TOOLS_ROOT%%/call.bat" start /B /WAIT "" "%%FFMPEG_TOOL_EXE%%" -i "%%FILE_IN%%"%%BARE_FLAGS%% -ss "%%TIME_START%%" -t "%%TIME_DUR%%" "%%FILE_OUT%%"
) else call "%%CONTOOLS_BUILD_TOOLS_ROOT%%/call.bat" start /B /WAIT "" "%%FFMPEG_TOOL_EXE%%" -i "%%FILE_IN%%" -map 0 -c copy%%BARE_FLAGS%% -ss "%%TIME_START%%" -t "%%TIME_DUR%%" "%%FILE_OUT%%"
