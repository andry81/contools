...
rem USAGE:
rem   ffmpeg_cut.bat [<flags>] [--] <input-file> <output-file> <start-time> <end-time>

rem Description:
rem   Video file split script. Uses a single file as an input to split by a
rem   criteria.
rem
...
rem <flags>:
rem   -dur
rem     The <end-time> is a duration time.
rem
...
rem
rem   -use-accurate-seek
rem     Uses an accurate seek to split any were between key frames.
...
rem 00:00:30.0
set "TIME_START=%~3"
rem 00:00:40.0 (or 00:00:10.0 if `-dur`)
set "TIME_END=%~4"
...
if %FLAG_DUR% EQU 0 (
  set FFMPEG_END_TIME_CMDLINE= -to "%TIME_END%"
) else set FFMPEG_END_TIME_CMDLINE= -t "%TIME_END%"
...
if %FLAG_NO_DEFAULT_FLAGS% NEQ 0 (
  call "%%CONTOOLS_BUILD_TOOLS_ROOT%%/call.bat"       start /B /WAIT "" "%%FFMPEG_EXE%%"%%BARE_FORWARD_FLAGS%% -ss "%%TIME_START%%" -i "%%FILE_IN%%"%%FFMPEG_COPYTS_CMDLINE%%%%BARE_FLAGS%%%%FFMPEG_SS_AFTER_I_CMDLINE%%%%FFMPEG_END_TIME_CMDLINE%% "%%FILE_OUT%%"
) else if %FLAG_ENABLE_REENCODE% NEQ 0 (
  call "%%CONTOOLS_BUILD_TOOLS_ROOT%%/call.bat"       start /B /WAIT "" "%%FFMPEG_EXE%%"%%BARE_FORWARD_FLAGS%% -ss "%%TIME_START%%" -i "%%FILE_IN%%" -map 0%%FFMPEG_COPYTS_CMDLINE%% -bsf:a aac_adtstoasc%%BARE_FLAGS%%%%FFMPEG_SS_AFTER_I_CMDLINE%%%%FFMPEG_END_TIME_CMDLINE%% "%%FILE_OUT%%"
) else call "%%CONTOOLS_BUILD_TOOLS_ROOT%%/call.bat"  start /B /WAIT "" "%%FFMPEG_EXE%%"%%BARE_FORWARD_FLAGS%% -ss "%%TIME_START%%" -i "%%FILE_IN%%" -map 0 -c copy%%FFMPEG_COPYTS_CMDLINE%% -bsf:a aac_adtstoasc%%BARE_FLAGS%%%%FFMPEG_SS_AFTER_I_CMDLINE%%%%FFMPEG_END_TIME_CMDLINE%% "%%FILE_OUT%%"
