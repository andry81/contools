...
set "R=!%~2!" & set "L1=0" & set "L2=0" & set "L3=0" & set "L4=0" & set "L5=0" & set "L6=0" & set "S=" ^
  & ( if defined R if "!R:~0,1!" == "+" set "R=!R:~1!" ) & ( if defined R if "!R:~0,1!" == "-" set "R=!R:~1!" & set "S=-" ) ^
...
set "L=" & set "F=0" & ( if "!L1!!L2!!L3!!L4!!L5!!L6!" == "000000" set "F=1" ) & ( if defined R call "%%~dp0ufoldpad.bat" L R && set "F=0" || set "L=" ) ^
  & ( if not defined L if !F! NEQ 0 set "S=" ) & ( if defined L set "L=!L!," ) 
for /F "tokens=* delims=" %%a in ("!S!!L!!L1!,!L2!,!L3!,!L4!,!L5!,!L6!") do endlocal & set "%~1=%%a" & exit /b %F%
exit /b 0

rem USAGE:
rem   ifoldpad6.bat <out-var> <var>

rem Description:
rem   Signed integer series fold script with 0 padding from the left if shorter
rem   than `N,N,N,N,N,N`.
rem
