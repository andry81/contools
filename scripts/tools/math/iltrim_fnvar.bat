...
set "L1=%%i" & set "L2=%%j" & set "L3=%%k" & set "L4=%%l" & set "L5=%%m" & set "L6=%%n" & set "F=%%o" & set "S=" ^
  & ( if defined L1 if "!L!:~0,1!" == "+" set "L1=!L1:~1!" ) & ( if defined L1 if "!L1:~0,1!" == "-" set "L1=!L1:~1!" & set "S=-" ) ^
...
set /A "L1+=0" & set /A "L2+=0" & set /A "L3+=0" & set /A "L4+=0" & set /A "L5+=0" & set /A "L6+=0" & set "L=!L1!,!L2!,!L3!,!L4!,!L5!,!L6!" ^
  & ( if "!L!" == "0,0,0,0,0,0" set "L=0" & if not defined F set "S=" ) ^
  & ( if not defined F for /F "tokens=* delims=" %%a in ("!S!!L!") do endlocal & endlocal & set "%~1=%%a" & exit /b 0 ) ^
  & ( if "!L!" NEQ "0" for /F "tokens=* delims=" %%a in ("!S!!L!,!F!") do endlocal & endlocal & set "%~1=%%a" & exit /b 0 ) & ^
for /F "tokens=* delims=" %%a in ("!S!!F!") do endlocal & set "R=%%a" & call "%%~0" L R & ^
setlocal ENABLEDELAYEDEXPANSION & for /F "tokens=* delims=" %%a in ("!L!") do endlocal & endlocal & set "%~1=%%a" & exit /b 0

rem USAGE:
rem   iltrim_fnvar.bat <out-var> <var>

rem Description:
rem   A signed integer number zero groups trim script to trim groups from
...
rem     number explicit normalization, then you have to call the `inorm.bat`
