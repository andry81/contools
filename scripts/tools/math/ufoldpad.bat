...
set "L=" & set "F=0" & ( if "!L1!!L2!!L3!!L4!!L5!!L6!" == "000000" set "F=1" ) & ( if defined R call "%%~0" L R && set "F=0" || set "L=" ) & ( if defined L set "L=!L!," ) & set "R=" ^
  & ( if defined L ( set "R=!L1!" ) else if !L1! NEQ 0 set "R=!L1!" ) & ( if defined R set "R=!R!," ) ^
...
for /F "tokens=* delims=" %%a in ("!L!!R!") do endlocal & set "%~1=%%a" & exit /b %F%
endlocal & set "%~1=0" & exit /b 1
exit /b 0

rem USAGE:
rem   ufoldpad.bat <out-var> <var>
...
rem
rem   Difference with the `ufold.bat` script is that it does not return an
rem   overflow and supports an extended sequence longer than `N,N,N,N,N,N`.
rem
rem   NOTE:
...
rem
rem   NOTE:
rem     The `ifoldpad.bat` version of the script has no sense, because a
rem     padding to the complete length `N,N,N,N,N,N` is required in case of a
rem     sign. Use `ifoldpad6*.bat` script(s) for a signed variant.
