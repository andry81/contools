...
set "L=" & set "F=0" & ( if "!L1!!L2!!L3!!L4!!L5!!L6!" == "000000" set "F=1" ) & ( if defined R call "%%~dp0ufoldpad.bat" L R && set "F=0" || set "L=" ) & ( if defined L set "L=!L!," )
for /F "tokens=* delims=" %%a in ("!L!!L1!,!L2!,!L3!,!L4!,!L5!,!L6!") do endlocal & set "%~1=%%a" & exit /b %F%
exit /b 0

rem USAGE:
rem   ufoldpad6.bat <out-var> <var>

rem Description:
rem   Unsigned integer series fold script with 0 padding from the left if
