...
for /F "tokens=* delims=" %%a in ("!L!!R!") do for /F "tokens=* delims=" %%i in ("!F!") do endlocal & set "%~1=%%a" & exit /b %%i
endlocal & set "%~1=0" & exit /b 1
exit /b 0

rem USAGE:
rem   foldpad.bat <out-var> <var>
...
rem   `foldpad6*.bat` scripts or `uadd.bat` script instead:
rem
rem     >
