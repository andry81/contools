...
set "L1=%%i" & set "L2=%%j" & set "L3=%%k" & set "L4=%%l" & set "L5=%%m" & set "L6=%%n" & set "F=%%o" & set "S=" ^
  ...
  & ( if defined F call "%%~dp0uunfold.bat" F F || call set /A "L6+=%%ERRORLEVEL%%" ) & ^
...
set /A "L6%%=1000" & set /A "L5%%=1000" & set /A "L4%%=1000" & set /A "L3%%=1000" & set /A "L2%%=1000" & set /A "L1%%=1000" ^
  & ( if "%%j" == "" set "L2=!L1!" & set "L1=0" ) ^
  & ( if "%%k" == "" set "L3=!L2!" & set "L2=!L1!" & set "L1=0" ) ^
  & ( if "%%l" == "" set "L4=!L3!" & set "L3=!L2!" & set "L2=!L1!" & set "L1=0" ) ^
  & ( if "%%m" == "" set "L5=!L4!" & set "L4=!L3!" & set "L3=!L2!" & set "L2=!L1!" & set "L1=0" ) ^
  & ( if "%%n" == "" set "L6=!L5!" & set "L5=!L4!" & set "L4=!L3!" & set "L3=!L2!" & set "L2=!L1!" & set "L1=0" ) ^
  ...
  & ( if defined R set "R=!S!!R!" ) & ( if !F! NEQ 0 set "F=!S!!F!" )
for /F "tokens=* delims=" %%a in ("!R!") do endlocal & set "%~1=%%a" & exit /b %F%
endlocal & set "%~1=0" & exit /b 0

rem USAGE:
rem   iunfold.bat <out-var> <var>

rem Description:
rem   Signed integer series unfold script.
rem
