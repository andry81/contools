...
set "L1=%%a" & set "L2=%%b" & set "L3=%%c" & set "L4=%%d" & set "L5=%%e" & set "L6=%%f" & set "F=%%g" & set "R=%~3" & set "S=" & set "FR=0" ^
  & ( if defined L1 if "!L1:~0,1!" == "+" set "L1=!L1:~1!" ) & ( if defined L1 if "!L1:~0,1!" == "-" set "S=-" & if defined F set "F=-!F!" ) ^
  & ( if defined F call "%%~0" F F "%%R%%" || call set /A "FR=%%ERRORLEVEL%%" ) & ^
set /A "L1*=R" & set /A "L2*=!S!R" & set /A "L3*=!S!R" & set /A "L4*=!S!R" & set /A "L5*=!S!R" & set /A "L6=!S!L6*R + FR" & ^
set /A "L5+=L6 / 1000" & set /A "L6%%=1000" & set /A "L4+=L5 / 1000" & set /A "L5%%=1000" & set /A "L3+=L4 / 1000" & set /A "L4%%=1000" & ^
set /A "L2+=L3 / 1000" & set /A "L3%%=1000" & set /A "L1+=L2 / 1000" & set /A "L2%%=1000" & set /A "F=L1 / 1000" & set /A "L1%%=1000" & set "S=" ^
  ...
  & ( if not defined S if !L6! LSS 0 ( set "S=-" ) else if !L6! GTR 0 set "S=+" ) & ( if defined S set "S=!S:+=!" ) & ^
for /F "tokens=1,2,3,4,5,6,7 delims=," %%a in ("!S!!L1:-=!,!L2:-=!,!L3:-=!,!L4:-=!,!L5:-=!,!L6:-=!,!F!") do endlocal & set "%~1=%%a,%%b,%%c,%%d,%%e,%%f" & exit /b %%g
endlocal & set "%~1=0,0,0,0,0,0" & exit /b 0

rem USAGE:
rem   imul.bat <out-var> <lvar> <rvalue>
...
rem   Must be less/greater than +/-2149634 for the `A6=999` excluding signed
rem   overflow in `Bn`.
rem   And must be less/greater than +/-2149634 for the rest `An=999` excluding
rem   signed overflow in `An+1`.
