...
rem Description:
rem   The `xcopy.exe`/`robocopy.exe` seamless wrapper script with `xcopy.exe`
...
rem CAUTION:
rem   The `xcopy.exe` has a file path limit up to 260 characters in a path. To
...
rem
rem   The `robocopy.exe` will copy hidden and archive files by default.
rem
...
rem
rem   -use_xcopy
rem     Use `xcopy.exe` utility instead of `robocopy.exe` utility.
...
rem
rem   -use_robocopy
rem     Use `robocopy.exe` utility instead of `xcopy.exe` utility.
...
call "%%CONTOOLS_ROOT%%/std/setshift.bat" %%FLAG_SHIFT%% XCOPY_FLAGS_ %%*

rem WORKAROUND: replace all quotes in flags to avoid accidental use as positional arguments
if defined XCOPY_FLAGS_ set "XCOPY_FLAGS_=%XCOPY_FLAGS_:"=%"
...
if defined XCOPY_FILE_BARE_FLAGS set "XCOPY_FLAGS=%XCOPY_FLAGS% %XCOPY_FILE_BARE_FLAGS%"
...
for %%i in (%XCOPY_FLAGS_%) do set "XCOPY_FLAG=%%i" & call :ROBOCOPY_FLAGS_CONVERT || exit /b -250

rem error if a flag does not start by `/` character
for %%i in (%XCOPY_FLAGS%) do set "XCOPY_FLAG=%%i" & call :XCOPY_FLAG_CHECK || exit /b -250

goto XCOPY_FLAG_CHECK_END

:XCOPY_FLAG_CHECK
if not "%XCOPY_FLAG:~0,1%" == "/" (
  echo;%?~%: error: xcopy flag is invalid: "%XCOPY_FLAG%"
  exit /b 1
) >&2

exit /b 0

:XCOPY_FLAG_CHECK_END
...
echo;^>^>"%SystemRoot%\System32\xcopy.exe" "%FROM_FILE_PATH_ABS%" "%TO_PATH_ABS%"%XCOPY_FLAGS% %XCOPY_EXCLUDES_CMD%
if exist "\\?\%TO_PATH_ABS%\%FROM_FILE%" (
  if %XCOPY_Y_FLAG_PARSED% NEQ 0 (
    set "XCOPY_INPUT=Y"
  ) else set "XCOPY_INPUT=N"
) else set "XCOPY_INPUT=F"

echo;%XCOPY_INPUT%|"%SystemRoot%\System32\xcopy.exe" "%FROM_FILE_PATH_ABS%" "%TO_PATH_ABS%"%XCOPY_FLAGS% %XCOPY_EXCLUDES_CMD%
set LAST_ERROR=%ERRORLEVEL%
...
if "%XCOPY_FLAG%" == "/S" set "XCOPY_DIR_RECUR=1" & exit /b 0
if "%XCOPY_FLAG%" == "/E" set "XCOPY_DIR_RECUR=1" & exit /b 0
...
set XCOPY_Y_FLAG_PARSED=0
for %%i in (%XCOPY_FLAGS_%) do set "XCOPY_FLAG=%%i" & call :XCOPY_FLAGS_CONVERT || exit /b -250
...
echo;^>^>"%SystemRoot%\System32\robocopy.exe" "%FROM_DIR_PATH_ABS%" "%TO_PATH_ABS%" "%FROM_FILE%" /R:0 /W:0 /NP /NJH /NS /NC /XX%ROBOCOPY_FLAGS%%ROBOCOPY_EXCLUDES_CMD%
"%SystemRoot%\System32\robocopy.exe" "%FROM_DIR_PATH_ABS%" "%TO_PATH_ABS%" "%FROM_FILE%" /R:0 /W:0 /NP /NJH /NS /NC /XX%ROBOCOPY_FLAGS%%ROBOCOPY_EXCLUDES_CMD%
