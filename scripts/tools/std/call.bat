...
rem USAGE:
rem   call.bat [-exe] [-lockfile <lock-file> [-trylock] [-lock-busy-wait-cmdline <lock-busy-wait-cmdline>]] [--] <cmdline>...
...
rem script flags
set /A "FLAG_SHIFT=0", "FLAG_EXE=0", "FLAG_TRYLOCK=0", "FLAG_LOCK_BUSY_WAIT_CMDLINE=50"
set "FLAG_LOCK_FILE="
...
rem CAUTION: must check on empty variable to avoid accidental `del /Q ""` case
if defined FLAG_LOCK_FILE del /F /Q /A:-D "%FLAG_LOCK_FILE%" >nul 2>nul
