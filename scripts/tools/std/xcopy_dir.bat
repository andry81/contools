...
rem
rem   Copies all content of <from-path> directory into <to-path> directory.
rem
...
if /i "%FROM_PATH_ABS%" == "%TO_PATH_ABS%" (
...
if %FLAG_TOUCH_DIR% EQU 0 (
  ...
  for /F "usebackq tokens=* delims="eol^= %%i in (`%%?.%%`) do set "TO_PATH=%%i" & call "%%CONTOOLS_ROOT%%/std/touch_dir.bat" "%%TO_PATH%%"
...
echo;^>^>"%SystemRoot%\System32\xcopy.exe" "%FROM_PATH_ABS%" "%TO_PATH_ABS%"%XCOPY_FLAGS%%XCOPY_EXCLUDES_CMD%
echo;D|"%SystemRoot%\System32\xcopy.exe" "%FROM_PATH_ABS%" "%TO_PATH_ABS%"%XCOPY_FLAGS%%XCOPY_EXCLUDES_CMD%
...
echo;^>^>"%SystemRoot%\System32\robocopy.exe" "%FROM_PATH_ABS%" "%TO_PATH_ABS%" /R:0 /W:0 /NP /NJH /NS /NC /XX%ROBOCOPY_FLAGS%%ROBOCOPY_EXCLUDES_CMD%
"%SystemRoot%\System32\robocopy.exe" "%FROM_PATH_ABS%" "%TO_PATH_ABS%" /R:0 /W:0 /NP /NJH /NS /NC /XX%ROBOCOPY_FLAGS%%ROBOCOPY_EXCLUDES_CMD%
