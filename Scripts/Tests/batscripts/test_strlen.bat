...
if %STRING_LEN% EQU %LASTERRORLEVEL% (
  ...
  "%TOOLS_PATH%\printf.exe" "PASSED: %__COUNTER1%: (%LASTERRORLEVEL% == %STRING_LEN%) STRING=`${__STRING__}`"
) else (
  rem print string containing __STRING__ environment variable value which may hold batch control characters
  "%TOOLS_PATH%\printf.exe" "FAILED: %__COUNTER1%: (%LASTERRORLEVEL% == %STRING_LEN%) STRING=`${__STRING__}`"
)
