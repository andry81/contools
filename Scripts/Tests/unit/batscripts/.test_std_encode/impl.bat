...
if "!STRING_INPUT!" EQU "!STRING_OUTPUT!" (
  "%CONTOOLS_UTILS_BIN_ROOT%/contools/printf.exe" "PASSED: %TEST_REPORT_PREFIX_STR%: ${TEST_FUNC}: STRING_INPUT=`${STRING_INPUT}`"
  exit /b 0
)

"%CONTOOLS_UTILS_BIN_ROOT%/contools/printf.exe" "FAILED: %TEST_REPORT_PREFIX_STR%: ${TEST_FUNC}: STRING_INPUT=`${STRING_INPUT}` STRING_OUTPUT=`${STRING_OUTPUT}`"
