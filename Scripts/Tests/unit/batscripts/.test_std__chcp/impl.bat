...
if "%CURRENT_CP_REF%" == "%CURRENT_CP%" if "%LAST_CP_REF%" == "%LAST_CP%" if "%CP_HISTORY_LIST_REF%" == "%CP_HISTORY_LIST%" (
  "%CONTOOLS_UTILITIES_BIN_ROOT%/printf.exe" "PASSED: %TEST_REPORT_PREFIX_STR%: CURRENT_CP=`${CURRENT_CP}` LAST_CP=`${LAST_CP}` CP_HISTORY_LIST=`${CP_HISTORY_LIST}`"
  set LASTERROR=0
  goto EXIT
)

"%CONTOOLS_UTILITIES_BIN_ROOT%/printf.exe" "FAILED: %TEST_REPORT_PREFIX_STR%: CURRENT_CP=`${CURRENT_CP}`==`${CURRENT_CP_REF}` LAST_CP=`${LAST_CP}`==`${LAST_CP_REF}` CP_HISTORY_LIST=`${CP_HISTORY_LIST}`==`${CP_HISTORY_LIST_REF}`"
set LASTERROR=1
