...
if %STRING_OFFSET% EQU %ERRORLEVEL_RETURNED% (
  ...
  "%CONTOOLS_UTILITIES_BIN_ROOT%/contools/printf.exe" "PASSED: %TEST_REPORT_PREFIX_STR%: STRING_OFFSET=%STRING_OFFSET% FOUND_CHAR=`${FOUND_CHAR}` STRING=`${__STRING__}` CHARS=`${__CHARS__}`"
  exit /b 0
)

rem print string containing __STRING__ and __CHARS__ environment variable value which may hold batch control characters
"%CONTOOLS_UTILITIES_BIN_ROOT%/contools/printf.exe" "FAILED: %TEST_REPORT_PREFIX_STR%: (%STRING_OFFSET% == %ERRORLEVEL_RETURNED%) FOUND_CHAR=`${FOUND_CHAR}` STRING=`${__STRING__}` CHARS=`${__CHARS__}`"
