...
if %STRING_LEN% EQU %ERRORLEVEL_RETURNED% (
  ...
  "%CONTOOLS_UTILITIES_BIN_ROOT%/contools/printf.exe" "PASSED: %TEST_REPORT_PREFIX_STR%: STRING_LEN=%STRING_LEN% STRING=`${__STRING__}`"
  exit /b 0
)

rem print string containing __STRING__ environment variable value which may hold batch control characters
"%CONTOOLS_UTILITIES_BIN_ROOT%/contools/printf.exe" "FAILED: %TEST_REPORT_PREFIX_STR%: (%ERRORLEVEL_RETURNED% == %STRING_LEN%) STRING=`${__STRING__}`"
