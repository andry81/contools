...
if %LASTERROR% NEQ 0 (
  ...
  call "%%CONTOOLS_ROOT%%/build/xcopy_dir.bat"  "%%TEST_DATA_REF_DIR_ROOT%%" "%%TEST_DATA_OUT_DIR%%\%%TEST_TEMP_DIR_NAME%%\ref" /Y /H /E
  call "%%CONTOOLS_ROOT%%/build/xcopy_file.bat" "%%TEST_TEMP_DIR%%" "%%TEST_DATA_OUTPUT_FILE_NAME%%" "%%TEST_DATA_OUT_DIR%%\%%TEST_TEMP_DIR_NAME%%" /Y /H /E
)
