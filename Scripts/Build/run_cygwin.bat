...
call "%%CONTOOLS_ROOT%%/setvarfromstd.bat" "%%CYGWIN_PATH%%\bin\cygpath.exe" -u "%%TEMP%%"
set "CYGDRIVE_TEMP=%STDOUT_VALUE%"

call "%%CONTOOLS_ROOT%%/setvarfromstd.bat" "%%CYGWIN_PATH%%\bin\cygpath.exe" -u "%%TMP%%"
...
:cecho
if exist "%CONTOOLS_TPOLAERT_CECHO_ROOT%/cecho.exe" (
  "%CONTOOLS_TPOLAERT_CECHO_ROOT%/cecho.exe" %*{#}{\n}
) else (
  echo.%*
)
exit /b
