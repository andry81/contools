...
call "%%~dp0Tools\query_session_id.bat" console "%%SERVER_USER%%" Conn
if %RETURN_VALUE% NEQ -1 goto FOUND

rem get logged in RDP session
call "%%~dp0Tools\query_session_id.bat" rdp "%%SERVER_USER%%" Conn
if %RETURN_VALUE% NEQ -1 goto FOUND
...
call "%%~dp0Tools\query_session_id.bat" services "" Conn
if %RETURN_VALUE% NEQ -1 goto FOUND
