...
[[ -n "$BASH" && (-z "$BASH_LINENO" || BASH_LINENO[0] -gt 0) && (-z "$SOURCE_CONTOOLS_TRAPLIB_SH" || SOURCE_CONTOOLS_TRAPLIB_SH -eq 0) ]] || return 0 || exit 0 # exit to avoid continue if the return can not be called
