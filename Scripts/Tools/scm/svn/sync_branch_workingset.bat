...
rem 2. If synchronization directory was exist before synchronization and had versioned/unversioned files and directories then script sometimes may leave
rem    some of these directories and files in a collision state (to findout just compare status files generated by the workingset generator script).
rem    If that happen then you have to cleanup that collision manually by revert the state of that directory or file and execute script again.
...
if not exist "%SYNC_BRANCH_PATH%\.svn\" (
  ...
  call :CMD svn co "%%BRANCH_WORKINGSET_DIR_URL%%" -r "%%BRANCH_WORKINGSET_INFO_CURRENT_REV%%" "%%SYNC_BRANCH_PATH%%" --depth infinity %%FLAG_TEXT_SVN_IGNORE_EXTERNALS%% --non-interactive || exit /b 41
  echo.
)
...
if %FLAG_SVN_AUTO_SWITCH% NEQ 0 (
  if not "%BRANCH_DIR_URL%" == "%BRANCH_WORKINGSET_DIR_URL%" (
    ...
    pushd "%SYNC_BRANCH_PATH%" && (
      call :CMD svn switch "%%BRANCH_WORKINGSET_DIR_URL%%" -r "%%BRANCH_WORKINGSET_INFO_CURRENT_REV%%" . --set-depth infinity %%FLAG_TEXT_SVN_IGNORE_EXTERNALS%% --non-interactive || ( popd & exit /b 55 )
      popd
    )
    echo.
  )
...
for /F "usebackq eol= tokens=1,* delims=|" %%i in ("%BRANCH_CHANGESET_FILE%") do (
  ...
  pushd "%SYNC_BRANCH_PATH%" && (
    call :CMD svn up "%%CHANGESET_PATH%%" -r "%%CHANGESET_REVISION%%" --depth infinity %%FLAG_TEXT_SVN_IGNORE_EXTERNALS%% --non-interactive || ( popd & exit /b 60 )
    popd
  )
  echo.
)
...
if %BRANCH_DIFF_FILESIZE% NEQ 0 pushd "%SYNC_BRANCH_PATH%" && (
  rem TODO:
  rem   Add PREPROCESS_EXTERNAL_REMOVE step, because update w/ --ignore-externals flag won't remove externals marked to remove
  rem   (externals marked to add will be added automatically in turn by it's explicit processing).
  rem Algorithm:
  rem   If patch file is not empty, then we have to request previous externals list and compare it with current one. Then
  rem   find removed externals and physically remove them. Additionally we have to remove parent directories if they were a part
  rem   of an external path and there is no items under version control (or just empty directory).
