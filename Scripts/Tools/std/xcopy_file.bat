...
if %XCOPY_FLAG_MOVE% EQU 0 (
  ...
  echo.^>^>move "%FROM_PATH_ABS%\%FROM_FILE%" "%TO_PATH_ABS%\%FROM_FILE%"
  move "%FROM_PATH_ABS%\%FROM_FILE%" "%TO_PATH_ABS%\%FROM_FILE%"
)
...
if "%XCOPY_FLAG%" == "/MOV" ( set "XCOPY_FLAG_MOVE=1" & exit /b 1 )
