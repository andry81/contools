...
) else call "%%~dp0cmake_pre_validate_vars.bat" %%3 %%4 %%5 %%6 %%7 %%8 %%9 || exit /b 10
...
if exist "%PROJECT_STAGE_POSTBUILD_ROOT.GEN_DIR%" (
  call "%%CONTOOLS_BUILD_TOOLS_ROOT%%/xcopy_dir.bat" "%%PROJECT_STAGE_POSTBUILD_ROOT.GEN_DIR%%" "%%%PROJECT_NAMESPACE%.PROJECT_STAGE_BUILD_ROOT.GEN_DIR%%/%%PROJECT_NAME%%%%PROJECT_DIR_SUFFIX%%" /E /Y /D || exit /b 20
)
rem pdb
if exist "%PROJECT_STAGE_POSTBUILD_ROOT.PDB_DIR%" (
  call "%%CONTOOLS_BUILD_TOOLS_ROOT%%/xcopy_dir.bat" "%%PROJECT_STAGE_POSTBUILD_ROOT.PDB_DIR%%" "%%%PROJECT_NAMESPACE%.PROJECT_STAGE_BUILD_ROOT.PDB_DIR%%/%%PROJECT_NAME%%%%PROJECT_DIR_SUFFIX%%" /E /Y /D || exit /b 21
)
rem lib
if exist "%PROJECT_STAGE_POSTBUILD_ROOT.LIB_DIR%" (
  call "%%CONTOOLS_BUILD_TOOLS_ROOT%%/xcopy_dir.bat" "%%PROJECT_STAGE_POSTBUILD_ROOT.LIB_DIR%%" "%%%PROJECT_NAMESPACE%.PROJECT_STAGE_BUILD_ROOT.LIB_DIR%%/%%PROJECT_NAME%%%%PROJECT_DIR_SUFFIX%%" /E /Y /D || exit /b 22
)
rem var
if exist "%PROJECT_STAGE_POSTBUILD_ROOT.VAR_DIR%" (
  call "%%CONTOOLS_BUILD_TOOLS_ROOT%%/xcopy_dir.bat" "%%PROJECT_STAGE_POSTBUILD_ROOT.VAR_DIR%%" "%%%PROJECT_NAMESPACE%.PROJECT_STAGE_BUILD_ROOT.VAR_DIR%%/%%PROJECT_NAME%%%%PROJECT_DIR_SUFFIX%%" /E /Y /D || exit /b 23
)
