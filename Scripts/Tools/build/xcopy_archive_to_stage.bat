...
call "%%CONTOOLS_ROOT%%/subtract_path.bat" "%%ARCHIVE_FROM_STAGE_DIR_ROOT%%" "%%COPY_FROM_STAGE_ROOT%%"
...
if exist "%XCOPY_ARCHIVE_EXCLUDE_FILES_FILE%" ( call "%%CONTOOLS_ROOT%%/std/del_file.bat" "%%XCOPY_ARCHIVE_EXCLUDE_FILES_FILE%%" /F /Q /A:-D >nul || exit /b 1 )
exit /b 0
