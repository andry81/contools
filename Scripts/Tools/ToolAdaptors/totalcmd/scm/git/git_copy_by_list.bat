...
if %FLAG_CONVERT_FROM_UTF16% NEQ 0 (
  ...
  rem
  call "%%CONTOOLS_ROOT%%/encoding/ansi2any.bat" UTF-16 UTF-8 "%%~1" > "%COPY_FROM_LIST_FILE_TMP%"
) else (
  set "COPY_FROM_LIST_FILE_TMP=%~1"
)
...
:GIT_COPY_FILE
if not defined TO_FILE_DIR_SUFFIX goto GIT_COPY_FILE_CMD
...
call :CMD git add "%%TO_FILE_PATH%%" || exit /b
