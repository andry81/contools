...
call "%%CONTOOLS_ROOT%%/build/call.bat" "%%CURL_EXECUTABLE%%" -v %%* "%%URL%%" -o "%%CURL_ADAPTOR_DOWNLOAD_TEMP_DIR%%/%%OUT_FILE_NAME%%" || exit /b

call "%%CONTOOLS_ROOT%%/build/xcopy_dir.bat"  "%%PROJECT_LOG_DIR%%"    "%%CURL_ADAPTOR_DOWNLOAD_TEMP_DIR%%/%%PROJECT_LOG_FILE_NAME_DATE_TIME%%" /Y /D /H || exit /b 10
