...
for %%i in (qBittorrent magnet) do (
  if defined FLAG_USE_CALLF_EXECUTABLE (
    call :CMD reg.exe add "HKLM\SOFTWARE\Classes\%%i\shell\open\command" /ve /t REG_SZ /d "\"%%FLAG_USE_CALLF_EXECUTABLE%%\" /ret-child-exit /no-subst-vars /pause-on-exit \"\" \"runas.exe /user:\\\"${QBITTORRENT_RUN_AS_USER}\\\" \\\"\\\\\\\"%%QBITTORRENT_EXECUTABLE_ESCAPED%%\\\\\\\" \\\\\\\"%%%%1\\\\\\\"\\\"\"" /f || exit /b 255
