...
if %NEST_LVL% EQU 0 if %LASTERROR% EQU 0 (
  ...
  call "%%CONTOOLS_ROOT%%/build/xcopy_dir.bat" "%%PROJECT_LOG_DIR%%" "%%GH_ADAPTOR_BACKUP_DIR%%/restapi/.log/%%PROJECT_LOG_DIR_NAME%%" /E /Y /D
)
...
if not defined SKIPPING_CMD (
  call "%%CONTOOLS_ROOT%%/build/call.bat" "backup_restapi_auth_user_repos_list.bat" owner || if %FLAG_EXIT_ON_ERROR% NEQ 0 exit /b 255
  echo.---
...
if not defined SKIPPING_CMD (
  call "%%CONTOOLS_ROOT%%/build/call.bat" "backup_restapi_auth_user_repos_list.bat" all || if %FLAG_EXIT_ON_ERROR% NEQ 0 exit /b 255
  echo.---
...
for /F "usebackq eol=# tokens=* delims=" %%i in ("%GITHUB_ADAPTOR_PROJECT_OUTPUT_CONFIG_ROOT%/accounts-user.lst") do (
  ...
  if not defined SKIPPING_CMD (
    call "%%CONTOOLS_ROOT%%/build/call.bat" "backup_restapi_user_repos_list.bat" "%%REPO_OWNER%%" owner || if %FLAG_EXIT_ON_ERROR% NEQ 0 exit /b 255
    echo.---
  ...
  if not defined SKIPPING_CMD (
    call "%%CONTOOLS_ROOT%%/build/call.bat" "backup_restapi_user_repos_list.bat" "%%REPO_OWNER%%" all || if %FLAG_EXIT_ON_ERROR% NEQ 0 exit /b 255
    echo.---
  ...
  if not defined SKIPPING_CMD (
    call "%%CONTOOLS_ROOT%%/build/call.bat" "backup_restapi_starred_repos_list.bat" "%%REPO_OWNER%%" || if %FLAG_EXIT_ON_ERROR% NEQ 0 exit /b 255
    echo.---
...
for /F "usebackq eol=# tokens=* delims=" %%i in ("%GITHUB_ADAPTOR_PROJECT_OUTPUT_CONFIG_ROOT%/accounts-org.lst") do (
  ...
  if not defined SKIPPING_CMD (
    call "%%CONTOOLS_ROOT%%/build/call.bat" "backup_restapi_org_repos_list.bat" "%%REPO_OWNER%%" sources || if %FLAG_EXIT_ON_ERROR% NEQ 0 exit /b 255
    echo.---
  ...
  if not defined SKIPPING_CMD (
    call "%%CONTOOLS_ROOT%%/build/call.bat" "backup_restapi_org_repos_list.bat" "%%REPO_OWNER%%" all || if %FLAG_EXIT_ON_ERROR% NEQ 0 exit /b 255
    echo.---
...
for /F "usebackq eol=# tokens=1,* delims=/" %%i in (%REPO_LISTS%) do (
  ...
  if not defined SKIPPING_CMD (
    call "%%CONTOOLS_ROOT%%/build/call.bat" "backup_restapi_user_repo_info.bat" "%%REPO_OWNER%%" "%%REPO%%" || if %FLAG_EXIT_ON_ERROR% NEQ 0 exit /b 255
    echo.---
  ...
  if %FLAG_QUERY_REPO_INFO_ONLY% EQU 0 (
    ...
    if not defined SKIPPING_CMD (
      call "%%CONTOOLS_ROOT%%/build/call.bat" "backup_restapi_repo_stargazers_list.bat" "%%REPO_OWNER%%" "%%REPO%%" || if %FLAG_EXIT_ON_ERROR% NEQ 0 exit /b 255
      echo.---
    ...
    if not defined SKIPPING_CMD (
      call "%%CONTOOLS_ROOT%%/build/call.bat" "backup_restapi_repo_subscribers_list.bat" "%%REPO_OWNER%%" "%%REPO%%" || if %FLAG_EXIT_ON_ERROR% NEQ 0 exit /b 255
      echo.---
    ...
    if not defined SKIPPING_CMD (
      call "%%CONTOOLS_ROOT%%/build/call.bat" "backup_restapi_repo_forks_list.bat" "%%REPO_OWNER%%" "%%REPO%%" || if %FLAG_EXIT_ON_ERROR% NEQ 0 exit /b 255
      echo.---
    ...
    if not defined SKIPPING_CMD (
      call "%%CONTOOLS_ROOT%%/build/call.bat" "backup_restapi_repo_releases_list.bat" "%%REPO_OWNER%%" "%%REPO%%" || if %FLAG_EXIT_ON_ERROR% NEQ 0 exit /b 255
      echo.---
