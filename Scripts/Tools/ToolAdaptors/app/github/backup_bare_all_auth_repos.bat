...
if %NEST_LVL% EQU 0 if %LASTERROR% EQU 0 (
  ...
  call "%%CONTOOLS_ROOT%%/build/xcopy_dir.bat" "%%PROJECT_LOG_DIR%%" "%%GH_ADAPTOR_BACKUP_DIR%%/bare/.log/%%PROJECT_LOG_DIR_NAME%%" /E /Y /D
)
...
for /F "usebackq eol=# tokens=1,* delims=/" %%i in ("%GITHUB_ADAPTOR_PROJECT_OUTPUT_CONFIG_ROOT%/repos-auth.lst") do (
  ...
  if not defined SKIPPING_CMD (
    call "%%CONTOOLS_ROOT%%/build/call.bat" "backup_bare_auth_repo.bat"%%BARE_FLAGS%% "%%REPO_OWNER%%" "%%REPO%%" || if %FLAG_EXIT_ON_ERROR% NEQ 0 exit /b 255
    echo.---
