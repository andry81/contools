...
set "GH_REPOS_BACKUP_TEMP_DIR=%GH_ADAPTOR_BACKUP_TEMP_DIR%/subscribers/%OWNER%/%REPO%"
set "GH_REPOS_BACKUP_DIR=%GH_ADAPTOR_BACKUP_DIR%/restapi/subscribers/%OWNER%/%REPO%"
...
:PAGE_LOOP
call set "GH_RESTAPI_REPO_SUBSCRIBERS_URL_PATH=%%GH_RESTAPI_REPO_SUBSCRIBERS_URL:{{OWNER}}=%OWNER%%%"
call set "GH_RESTAPI_REPO_SUBSCRIBERS_URL_PATH=%%GH_RESTAPI_REPO_SUBSCRIBERS_URL_PATH:{{REPO}}=%REPO%%%"

set "GH_RESTAPI_REPO_SUBSCRIBERS_URL_PATH=%GH_RESTAPI_REPO_SUBSCRIBERS_URL_PATH%?per_page=%GH_RESTAPI_PARAM_PER_PAGE%&page=%PAGE%"

set "CURL_OUTPUT_FILE=%GH_REPOS_BACKUP_TEMP_DIR%/%GH_RESTAPI_REPO_SUBSCRIBERS_FILE%"

call set "CURL_OUTPUT_FILE=%%CURL_OUTPUT_FILE:{{PAGE}}=%PAGE%%%"

call :CURL "%%GH_RESTAPI_REPO_SUBSCRIBERS_URL_PATH%%" || goto MAIN_EXIT
echo.
...
call "%%CONTOOLS_BUILD_TOOLS_ROOT%%/add_files_to_archive.bat" "%%GH_ADAPTOR_BACKUP_TEMP_DIR%%" "*" "%%GH_REPOS_BACKUP_DIR%%/subscribers--[%%OWNER%%][%%REPO%%]--%%PROJECT_LOG_FILE_NAME_DATE_TIME%%.7z" -sdel || exit /b 20
echo.
