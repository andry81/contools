...
if %NO_DEFAULT_FLAGS% NEQ 0 (
  call :CMD start /B /WAIT "" "%%FFMPEG_TOOL_EXE%%" -i "%%FILE_IN%%"%%BARE_FLAGS%% "%%FILE_OUT%%"
) else if %ENABLE_REENCODE% NEQ 0 (
  call :CMD start /B /WAIT "" "%%FFMPEG_TOOL_EXE%%" -err_detect ignore_err -i "%%FILE_IN%%"%%BARE_FLAGS%% "%%FILE_OUT%%"
) else call :CMD start /B /WAIT "" "%%FFMPEG_TOOL_EXE%%" -err_detect ignore_err -i "%%FILE_IN%%" -c copy%%BARE_FLAGS%% "%%FILE_OUT%%"
exit /b
