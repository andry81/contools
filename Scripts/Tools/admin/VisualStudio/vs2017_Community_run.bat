...
if not defined VS2017COMM_DOWNLOAD_CACHE_DIR (
  echo.%~nx0: error: VS2017COMM_DOWNLOAD_CACHE_DIR directory path is not defined.
  exit /b 5
) >&2

"%VS2017COMM_LAYOUT_ROOT%\%VS2017COMM_LAYOUT_DIR%\vs_professional.exe" --add Microsoft.VisualStudio.Workload.NativeDesktop --includeRecommended --path cache="%VS2017COMM_DOWNLOAD_CACHE_DIR%" %*
