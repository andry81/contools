...
pushd "%SOURCES_DIR%" && (
  if exist "%SOURCES_DIR%\help.def" "%BUILD_TOOLS_DIR%\msys\bin\autogen.exe"%AUTOGEN_BARE_FLAGS% -L "%SOURCES_DIR%" help.def
  "%BUILD_TOOLS_DIR%\msys\bin\autogen.exe"%AUTOGEN_BARE_FLAGS% -L "%SOURCES_DIR%" version.def
  popd
) || exit /b 65534
...
if exist "%CONSOLE_HELP_FILE%.txt" (
  ...
  call :CMD "%CONTOOLS_UTILITIES_BIN_ROOT%/contools/thlibautocfg.exe" -txt2c %THLIBAUTOCFG_CMD_LINE%"%CONSOLE_HELP_FILE%.txt" "%CONSOLE_HELP_FILE%_inl.hpp"
)
