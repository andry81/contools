...
#include "help_inl.hpp"
  ...
  if (app_args.fmt_str && argc >= 4) {
    ...
    for (int i = 0; i < argc - 3; i++) {
      ...
      if (strcmp(app_args.args[i] , "")) {
        _parse_string(app_args.args[i], app_out_args.args[i], value_in_str);
      } else {
        app_args.args[i] = 0;
      }
    }
  ...
  if (cmd_args.fmt_str && argc >= 5) {
    ...
    for (int i = 0; i < argc - 3; i++) {
      ...
      if (strcmp(cmd_args.args[i], "")) {
        _parse_string(cmd_args.args[i], cmd_out_args.args[i], value_in_str);
      }
      else {
        cmd_args.args[i] = 0;
      }
  ...
  if (app_args.fmt_str) {
    _parse_string(app_args.fmt_str, app_out_args.fmt_str, value_in_str, app_args, app_out_args);
  }
  if (cmd_args.fmt_str) {
    _parse_string(cmd_args.fmt_str, cmd_out_args.fmt_str, value_in_str, cmd_args, cmd_out_args);
  }
