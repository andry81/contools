...
Options::Options()
{
    ...
    shell_exec_method = ShellExecMethod_None;
    unelevate_method = UnelevationMethod_None;
...
void Options::merge(const Options & options)
{
    ...
    MERGE_OPTION(options, shell_exec_method, ShellExecMethod_None);
    MERGE_OPTION(options, unelevate_method, UnelevationMethod_None);
...
int ExecuteProcess(LPCTSTR app, size_t app_len, LPCTSTR cmd, size_t cmd_len)
{
    ...
    g_no_std_inherit = g_is_child_process_elevating || g_flags.no_std_inherit || is_idle_execute;
    ...
    g_pipe_stdin_to_stdout = !g_is_child_process_elevating && (g_flags.pipe_stdin_to_stdout || is_idle_execute);
    ...
    [&]() { if_break(true) __try {
        ...
        const bool is_unelevating = g_is_child_process_unelevating &&
            ( g_flags.shell_exec_unelevate && g_options.shell_exec_method == ShellExecMethod_UnelevateFromExplorer ||
              g_flags.unelevate && g_options.unelevate_method == UnelevationMethod_ShellExecuteFromExplorer );
        ...
        const bool do_wait_child = !is_idle_execute && (!is_unelevating ?
        ...
        if (!is_idle_execute) {
            ...
            if (!is_shell_exec && !g_is_this_process_self_elevating) {
                ...
                if (!g_is_child_process_unelevating) {
        ...
        if (!is_idle_execute) {
            if (is_child_executed) {
                ...
                if (is_unelevating && g_flags.print_shell_error_string) {
    ...
    const Flags & promote_or_demote_child_flags, const Options & promote_or_demote_child_options)
{
    ...
    if (g_is_child_process_elevating) { // just in case
...
#error to be continued...
    ...
    //change_current_dir // passes directly into an API call
