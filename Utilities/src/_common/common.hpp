...
namespace {
    ...
    const char * _extract_variable(const char * last_offset, const char * parse_str, std::string & parsed_str, char * env_buf)
    {
        ...
        if_break(p_in_str_var) {
            ...
            const DWORD env_buf_size = !in_str_var_name.empty() ? ::GetEnvironmentVariableA(in_str_var_name.c_str(), env_buf, MAX_ENV_BUF_SIZE) : (MAX_ENV_BUF_SIZE + 1);
